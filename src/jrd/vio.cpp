...
bool SweepTask::handler(WorkItem& _item)
{
	...
	try
	{
			...
			relation->getPages(tdbb)->rel_pages)
		{
			...
			{
				string str;
				str.printf("Acquire garbage collection lock failed (%s)", relation->rel_name.toString().c_str());
...
inline void check_gbak_cheating_insupd(thread_db* tdbb, const jrd_rel* relation, const char* op)
{
		...
		!request->hasInternalStatement())
	{
		status_exception::raise(Arg::Gds(isc_protect_sys_tab) <<
			Arg::Str(op) << relation->rel_name.toString());
	}
}
					...
					(id = MOV_get_long(tdbb, &desc2, 0)))
				{
					if (!(file_flags & FILE_inactive))
					{
						if (file_flags & FILE_nodelete)
							DFW_post_work(transaction, dfw_delete_shadow_nodelete, &desc, nullptr, id);
						else
							DFW_post_work(transaction, dfw_delete_shadow, &desc, nullptr, id);
					}
				}
			}
			break;
 ...
 **************************************/
	...
	{
		USHORT fieldId;
		std::optional<USHORT> dependencyId;
	};
	...
	static const std::unordered_map<USHORT, std::vector<SchemaFieldDependency>> schemaFields = {
		...
		{rel_indices, {
			{f_idx_schema},
			{f_idx_foreign_schema, f_idx_foreign}
		}},
		{rel_rfr, {
			{f_rfr_schema},
			{f_rfr_field_source_schema, f_rfr_sname}
		}},
		{rel_relations, {
			{f_rel_schema}
		}},
		{rel_vrel, {
			{f_vrl_schema},
			{f_vrl_rname_schema, f_vrl_rname}
		}},
		{rel_triggers, {
			{f_trg_schema}
		}},
		...
		{rel_args, {
			{f_arg_schema},
			{f_arg_rel_schema, f_arg_rname},
			{f_arg_field_source_schema, f_arg_sname}
		}},
		{rel_msgs, {
			{f_msg_schema}
		}},
		...
		{rel_prc_prms, {
			{f_prm_schema},
			{f_prm_rel_schema, f_prm_rname},
			{f_prm_field_source_schema, f_prm_sname}
		}},
		{rel_charsets, {
			{f_cs_schema},
			{f_cs_def_coll_schema, f_cs_def_collate}
		}},
		{rel_collations, {
			{f_coll_schema},
			{f_coll_base_collation_schema, f_coll_base_collation_name}	// FIXME: from external?
		}},
		{rel_exceptions, {
			{f_xcp_schema}
		}},
		...
		{rel_pub_tables, {
			{f_pubtab_tab_schema, f_pubtab_tab_name}
		}},
	};
	...
	{
		for (const auto [fieldId, dependencyId] : relSchemaFields->second)
		{
			if (rpb->rpb_record->isNull(fieldId) &&
				(!dependencyId.has_value() || !rpb->rpb_record->isNull(dependencyId.value())) &&
				!EVL_field(0, rpb->rpb_record, fieldId, &schemaDesc))
			{
				if (desc.isNull())
					desc.makeText(strlen(DEFAULT_SCHEMA), CS_METADATA, (UCHAR*) DEFAULT_SCHEMA);
	...
	if (relation->rel_id == rel_priv)
	{
		...
		for (const auto [fieldId, objTypeId] : privSchemaFields)
		{
			if (rpb->rpb_record->isNull(fieldId) &&
				!rpb->rpb_record->isNull(objTypeId) &&
				EVL_field(0, rpb->rpb_record, objTypeId, &desc2))
			{
				...
				switch (objType)
				{
					...
					case obj_package_body:
						EVL_field(0, rpb->rpb_record, fieldId, &desc2);
			...
			protect_system_table_insert(tdbb, request, relation);
			{
				...
				else
				{
						...
						((rel_flags = MOV_get_long(tdbb, &desc2, 0)) & FILE_difference))
					{
						...
						DFW_post_work(transaction, dfw_add_file, &desc, nullptr, 0);
					}
				}
			}
			break;
 ...
 **************************************/
	...
	status_exception::raise(Arg::Gds(isc_protect_sys_tab) <<
			Arg::Str("INSERT") << relation->rel_name.toString());
}
 ...
 **************************************/
	...
	status_exception::raise(Arg::Gds(isc_protect_sys_tab) <<
		Arg::Str(operation) << relation->rel_name.toString());
}
