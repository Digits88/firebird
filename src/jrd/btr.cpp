...
void IndexErrorContext::raise(thread_db* tdbb, idx_e result, Record* record)
{
	...
	case idx_e_keytoobig:
		ERR_post_nothrow(Arg::Gds(isc_imp_exc) <<
						 Arg::Gds(isc_keytoobig) << indexName.toString());
		break;

	case idx_e_foreign_target_doesnt_exist:
		ERR_post_nothrow(Arg::Gds(isc_foreign_key) <<
						 constraintName.toQuotedString() << relationName.toString() <<
	...
	case idx_e_duplicate:
		if (haveConstraint)
		{
			...
			ERR_post_nothrow(Arg::Gds(isc_no_dup) << indexName.toString());
		break;
 ...
 **************************************/
	...
	if (error)
	{
		...
		if (indexName.object.hasData())
			status.assign(Arg::Gds(error) << indexName.toString());
		else
			// there is no index in table @1 with id @2
			status.assign(Arg::Gds(isc_indexnotdefined) << relation->rel_name.toString() << Arg::Num(idx->idx_id));
