...
public:
	...
	{
		if (name.object.hasData() && name.schema.isEmpty())
		{
			if (csb_schema.hasData())
				name.schema = csb_schema;
			else
				tdbb->getAttachment()->qualifyExistingName(tdbb, name, objType);
		}
	}
	...
	MetaName csb_schema;	// FIXME: set in all neccessary places
