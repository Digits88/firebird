...
void ProfilerPlugin::flush(ThrowStatusExceptionWrapper* status)
{
	constexpr auto sessionSql = R"""(
		update or insert into system.plg$prof_sessions
	...
	constexpr auto statementSql = R"""(
		update or insert into system.plg$prof_statements
		    (profile_id, statement_id, parent_statement_id, statement_type, schema_name, package_name, routine_name,
		     sql_text)
		    values (?, ?, ?, ?, ?, ?, ?)
	...
	constexpr auto cursorSql = R"""(
		update or insert into system.plg$prof_cursors
	...
	constexpr auto recSrcSql = R"""(
		update or insert into system.plg$prof_record_sources
	...
	constexpr auto requestSql = R"""(
		update or insert into system.plg$prof_requests
	...
	constexpr auto recSrcStatSql = R"""(
		execute block (
		    profile_id type of column system.plg$prof_record_source_stats.profile_id = ?,
		    statement_id type of column system.plg$prof_record_source_stats.statement_id = ?,
		    request_id type of column system.plg$prof_record_source_stats.request_id = ?,
		    cursor_id type of column system.plg$prof_record_source_stats.cursor_id = ?,
		    record_source_id type of column system.plg$prof_record_source_stats.record_source_id = ?,
		    open_counter type of column system.plg$prof_record_source_stats.open_counter = ?,
		    open_min_elapsed_time type of column system.plg$prof_record_source_stats.open_min_elapsed_time = ?,
		    open_max_elapsed_time type of column system.plg$prof_record_source_stats.open_max_elapsed_time = ?,
		    open_total_elapsed_time type of column system.plg$prof_record_source_stats.open_total_elapsed_time = ?,
		    fetch_counter type of column system.plg$prof_record_source_stats.fetch_counter = ?,
		    fetch_min_elapsed_time type of column system.plg$prof_record_source_stats.fetch_min_elapsed_time = ?,
		    fetch_max_elapsed_time type of column system.plg$prof_record_source_stats.fetch_max_elapsed_time = ?,
		    fetch_total_elapsed_time type of column system.plg$prof_record_source_stats.fetch_total_elapsed_time = ?
		)
		as
		begin
		    merge into system.plg$prof_record_source_stats
	...
	constexpr auto psqlStatSql = R"""(
		execute block (
		    profile_id type of column system.plg$prof_psql_stats.profile_id = ?,
		    statement_id type of column system.plg$prof_psql_stats.statement_id = ?,
		    request_id type of column system.plg$prof_psql_stats.request_id = ?,
		    line_num type of column system.plg$prof_psql_stats.line_num = ?,
		    column_num type of column system.plg$prof_psql_stats.column_num = ?,
		    counter type of column system.plg$prof_psql_stats.counter = ?,
		    min_elapsed_time type of column system.plg$prof_psql_stats.min_elapsed_time = ?,
		    max_elapsed_time type of column system.plg$prof_psql_stats.max_elapsed_time = ?,
		    total_elapsed_time type of column system.plg$prof_psql_stats.total_elapsed_time = ?
		)
		as
		begin
		    merge into system.plg$prof_psql_stats
		        using system.rdb$database on
		            profile_id = :profile_id and
		            statement_id = :statement_id and
		            request_id = :request_id and
	...
	RefPtr<ITransaction> transaction)
{
	constexpr const char* createSqlStaments[] = {
		...
		"create sequence system.plg$prof_profile_id",

		"grant usage on sequence system.plg$prof_profile_id to plg$profiler",

		R"""(
		create table system.plg$prof_sessions (
		...
		"grant select, update, insert, delete on table system.plg$prof_sessions to plg$profiler",

		R"""(
		create table system.plg$prof_statements (
		    profile_id bigint not null
		        constraint plg$prof_statements_session_fk
		            references system.plg$prof_sessions
		            on delete cascade
		    ...
		    constraint plg$prof_statements_parent_statement_fk
		        ...
		        references system.plg$prof_statements (profile_id, statement_id)
		        on delete cascade
		        using index plg$prof_statements_parent_statement
		))""",

		"grant select, update, insert, delete on table system.plg$prof_statements to plg$profiler",

		R"""(
		create table system.plg$prof_cursors (
		    profile_id bigint not null
		    ...
		    constraint plg$prof_cursors_statement_fk
		        foreign key (profile_id, statement_id) references system.plg$prof_statements
		        on delete cascade
		        using index plg$prof_cursors_profile_statement
		))""",

		"grant select, update, insert, delete on table system.plg$prof_cursors to plg$profiler",

		R"""(
		create table system.plg$prof_record_sources (
		    profile_id bigint not null
		    ...
		    constraint plg$prof_record_sources_cursor_fk
		        foreign key (profile_id, statement_id, cursor_id) references system.plg$prof_cursors
		        on delete cascade
		    ...
		    constraint plg$prof_record_sources_parent_record_source_fk
		        ...
		        references system.plg$prof_record_sources (profile_id, statement_id, cursor_id, record_source_id)
		        on delete cascade
		        using index plg$prof_record_sources_profile_statement_cursor_parent_rec_src
		))""",

		"grant select, update, insert, delete on table system.plg$prof_record_sources to plg$profiler",

		R"""(
		create table system.plg$prof_requests (
		    profile_id bigint not null
		    ...
		    constraint plg$prof_requests_caller_statement_fk
		        foreign key (profile_id, caller_statement_id) references system.plg$prof_statements
		        on delete cascade
		    ...
		    constraint plg$prof_requests_caller_request_fk
		        ...
		        references system.plg$prof_requests (profile_id, statement_id, request_id)
		        on delete cascade
		        using index plg$prof_requests_profile_caller_statement_caller_request
		))""",

		"grant select, update, insert, delete on table system.plg$prof_requests to plg$profiler",

		R"""(
		create table system.plg$prof_psql_stats (
		    profile_id bigint not null
		    ...
		    constraint plg$prof_psql_stats_request_fk
		        foreign key (profile_id, statement_id, request_id) references system.plg$prof_requests
		        on delete cascade
		...
		"grant select, update, insert, delete on table system.plg$prof_psql_stats to plg$profiler",

		R"""(
		create table system.plg$prof_record_source_stats (
		    profile_id bigint not null
		    ...
		    constraint plg$prof_record_source_stats_record_source_fk
		        ...
		        references system.plg$prof_record_sources
		        on delete cascade
		        using index plg$prof_record_source_stats_statement_cursor_record_source
		))""",

		"grant select, update, insert, delete on table system.plg$prof_record_source_stats to plg$profiler"
	};
	...
	RefPtr<ITransaction> transaction)
{
	constexpr const char* sqlStaments[] = {
		R"""(
		alter table system.plg$prof_statements
...
void ProfilerPlugin::recreateViews(ThrowStatusExceptionWrapper* status, RefPtr<IAttachment> attachment,
	RefPtr<ITransaction> transaction)
{
	constexpr const char* sqlStaments[] = {
		R"""(
		recreate view system.plg$prof_statement_stats_view
		as
		select req.profile_id,
		       ...
		       (select sql_text
		          from system.plg$prof_statements
		  ...
		  from system.plg$prof_requests req
		  join system.plg$prof_statements sta
		    on sta.profile_id = req.profile_id and
		       sta.statement_id = req.statement_id
		  left join system.plg$prof_statements sta_parent
		...
		"grant select on table system.plg$prof_statement_stats_view to plg$profiler",

		R"""(
		recreate view system.plg$prof_psql_stats_view
		as
		select pstat.profile_id,
		  ...
		  from system.plg$prof_psql_stats pstat
		  join system.plg$prof_statements sta
		...
		"grant select on table system.plg$prof_psql_stats_view to plg$profiler",

		R"""(
		recreate view system.plg$prof_record_source_stats_view
		as
		select rstat.profile_id,
		  ...
		  from system.plg$prof_record_source_stats rstat
		  join system.plg$prof_cursors cur
		    on cur.profile_id = rstat.profile_id and
		       cur.statement_id = rstat.statement_id and
		       cur.cursor_id = rstat.cursor_id
		  join system.plg$prof_record_sources recsrc
		...
		"grant select on table system.plg$prof_record_source_stats_view to plg$profiler"
	};
...
void ProfilerPlugin::loadMetadata(ThrowStatusExceptionWrapper* status)
{
	constexpr auto loadObjectsSql =
		R"""(
		select *
		    from system.plg$prof_sessions
		    cross join system.plg$prof_statements
		    cross join system.plg$prof_record_sources
		    cross join system.plg$prof_requests
		    cross join system.plg$prof_psql_stats
		    cross join system.plg$prof_record_source_stats
		    where next value for system.plg$prof_profile_id = 0
		)""";
	  ...
	  description(defaultPool(), aDescription)
{
	...
	constexpr auto sequenceSql = "select next value for system.plg$prof_profile_id from system.rdb$database";
