 ...
 **************************************/
		...
		if (field->typeOfTable.object.hasData())
		{
			...
			if (!fld)
			{
				...
				post_607(Arg::Gds(isc_dyn_column_does_not_exist) <<
						 		field->typeOfName.toString() <<
								field->typeOfTable.toString());
			}
		}
		else
		{
			...
			if (!METD_get_domain(dsqlScratch->getTransaction(), field, field->typeOfName))
			{
				...
				post_607(Arg::Gds(isc_dsql_domain_not_found) << field->typeOfName.toString());
			}
		}
	...
	if (charset_name.object.hasData())
	{
		...
		if (!resolved_charset)
		{
			...
			ERRD_post(Arg::Gds(isc_sqlerr) << Arg::Num(-204) <<
					  Arg::Gds(isc_dsql_datatype_err) <<
                      Arg::Gds(isc_charset_not_found) << charset_name.toString());
		}
	...
	if (collation_name.object.hasData())
	{
		...
		if (!resolved_collation)
		{
			...
			ERRD_post(Arg::Gds(isc_sqlerr) << Arg::Num(-204) <<
                      ...
                      Arg::Gds(isc_collation_not_found) << collation_name.toString() << charSetName.toString());
		}
			...
			(field->charSetId.value_or(CS_NONE) != ttype_dynamic))
		{
			ERRD_post(Arg::Gds(isc_sqlerr) << Arg::Num(-204) <<
					  Arg::Gds(isc_dsql_datatype_err) <<
                      Arg::Gds(isc_collation_not_for_charset) << collation_name.toString());
		}
