...
// FIXME: select cast('x' as varchar(10) character set system.utf8 collate fix) from rdb$database;
ValueExprNode* CastNode::dsqlPass(DsqlCompilerScratch* dsqlScratch)
{
	...
	DDL_resolve_intl_type(dsqlScratch, node->dsqlField, dummyCollationName);
	node->setParameterType(dsqlScratch, NULL, false);
	...
	StreamType* map, jrd_fld* fld)
{
	if (fld->fld_generator_name.object.hasData())
	{
		...
		if (!MET_load_generator(tdbb, genNode->generator, &sysGen, &genNode->step))
			status_exception::raise(Arg::Gds(isc_gennotdef) << fld->fld_generator_name.toString());
		...
		if (sysGen)
		{
			status_exception::raise(
				...
				"generator" <<
				fld->fld_generator_name.toString());
		}
...
ValueExprNode* FieldNode::internalDsqlPass(DsqlCompilerScratch* dsqlScratch, RecordSourceNode** list)
{
    ...
    */
	...
	if (!node && !(list && *list))
		PASS1_field_unknown(dsqlQualifier.toString().nullStr(), dsqlName.toQuotedString().nullStr(), this);
	...
	if (field->fld_computation && !relation->rel_view_rse)
	{
		if (csb->csb_currentAssignTarget == this)
		{
			...
			ERR_post(
				...
				(relation->rel_name.toString() + "." + field->fld_name.toQuotedString()));
		}
...
dsc* GenIdNode::execute(thread_db* tdbb, Request* request) const
{
	...
	if (sysGen && change != 0)
	{
		if (!request->hasInternalStatement() && !tdbb->getAttachment()->isRWGbak())
			status_exception::raise(Arg::Gds(isc_cant_modify_sysobj) << "generator" << generator.name.toString());
	}
...
void InternalInfoNode::make(DsqlCompilerScratch* /*dsqlScratch*/, dsc* desc)
{
	...
	switch (infoType)
	{
		...
		case INFO_TYPE_EXCEPTION:
			// FIXME: must increase length - considering schema and escapes - here and in getDesc
...
dsc* InternalInfoNode::execute(thread_db* tdbb, Request* request) const
{
	...
	else if (infoType == INFO_TYPE_EXCEPTION)
	{
		...
		const auto xcpNameStr = xcpName.toString();
	...
	else
	{
		...
		if (charLength > MAX_STR_SIZE / charSet->maxBytesPerChar())
		{
			ERRD_post(Arg::Gds(isc_sqlerr) << Arg::Num(-104) <<
					  ...
					  METD_get_charset_name(dsqlScratch->getTransaction(), constant->litDesc.getCharSet()).toString());
		}
		else
...
ValueExprNode* RecordKeyNode::dsqlPass(DsqlCompilerScratch* dsqlScratch)
{
	...
	{
		// Field unresolved.
		PASS1_field_unknown(dsqlQualifier.toString().nullStr(), getAlias(false), this);
	}
...
void RecordKeyNode::raiseError(dsql_ctx* context) const
{
	...
	if (alias.object.hasData() && name != alias.toString())
	{
		if (name.hasData())
			name += " (alias " + alias.toString() + ")";
		else
			name = alias.toString();
	}
	...
	const UCHAR /*blrOp*/)
{
	...
	if (name == "MAKE_DBKEY")
	{
		...
		if (literal && literal->litDesc.isText())
		{
			...
			auto relName = QualifiedName::parse(string(reinterpret_cast<const char*>(ptr), len));
			tdbb->getAttachment()->qualifyExistingName(tdbb, relName, obj_relation);
	...
	if (!function->isImplemented())
	{
		status_exception::raise(
			Arg::Gds(isc_func_pack_not_implemented) <<
				Arg::Str(function->getName().object) << function->getName().getSchemaAndPackage().toString());
	}
...
ValueExprNode* UdfCallNode::dsqlPass(DsqlCompilerScratch* dsqlScratch)
{
	...
	if (!function)
	{
		ERRD_post(Arg::Gds(isc_sqlerr) << Arg::Num(-804) <<
				  ...
				  Arg::Gds(isc_random) << name.toString());
	}
	...
	if (function->udf_private && function->udf_name.getSchemaAndPackage() != dsqlScratch->package)
	{
		status_exception::raise(
			...
			function->udf_name.getSchemaAndPackage().toString());
	}
